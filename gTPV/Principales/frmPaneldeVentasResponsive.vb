Public Class frmPaneldeVentasResponsive
...
#Region "Manejadores"
        ...
        Select Case app
            ...
            Case "FINALIZA_TICKET"
                If My.MyHardware.cashlogy_sw Then
                    ...
                    'C#’=Comando a cobrar’#1’=Número de operación’#1’= Código de caja’#1000’=Importe a cobrar’#0’=Sin segunda pantalla’#0’=Posición X de segunda pantalla’#0’=Posición Y de segunda pantalla’#0’=Sin botón aceptar’#0’=Sin cobros parciales’#1’=Pantalla siempre por delante’#0’=Sin céntimos manuales’#0=’Sin cobro manual’#
                    'Respuesta de Cashlogy Connector  #0#2000#1000#0#0#
                    '#0’=Sin error en ejecución’#2000’=importe admitido’#1000’=Importe devuelto ’#0’No ha habido cobro manual’#0’No se han añadido cambios en la operación#

                    Dim str As String = ""
                    str = "#C"
                    str &= "#" & Format(Now, "yyyyMMddHHss") 'numero operacion
                    str &= "#" & Format(Now, "yyyyMMddHHss") 'Codigo caja
                    str &= "#" & Format(Me._DblTotal * 100, "00") 'Importe

                    My.cashlogy_Send(str & "#0#0#0#0#1#1#0#0#")
                    'My.cashlogy_Send("#C#1#1#1000#0#0#0#0#1#1#0#0#")
                    str = My.cashlogy_read()
                    If IsNothing(str) Then Exit Sub
                    If str.Substring(0, 11) = "#WR:CANCEL#" Then Exit Sub

                    'MsgBox("Cobro cashlogic")
                    'Exit Sub
                End If
            ...
            Case "CANCELATICKET"
                ...
                If My.m_opt.modo_seguro AndAlso Me.idRef >= 0 Then
                    ...
                    If Not (idUser = -3 OrElse idUser = 99) Then
                        ...
                        If Not rst("tipo_user").Value = "MAESTRO" AndAlso Not rst("tipo_user").Value = "ADMINISTRADOR" Then
                            MsgBox("OPCIÓN NO permitida por tipo de usuario.", MsgBoxStyle.Critical)
                            Exit Sub
                        End If
            ...
            Case "DEL_LINEA"
                'Compruebo si tengo control de acceso
                'If My.m_opt.modo_seguro_secciones And Me.idRef >= 0 Then
                If Me.idRef >= 0 Then
                ...
                ' LOG
                'My.m_db.Execute("INSERT INTO logs_del(str, fh, sw, ref) VALUES(" & Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.name).Text & " - " & Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.total).Text & "','" & Format(Now, "dd/MM/yyyy HH:mm:ss") & "', " & IIf(Me.LstLines.SelectedItems(0).Text > 0, True, False) & ", " & Me.idRef & ")")
                'My.m_db.Execute("INSERT INTO logs_del(user, t, str, fh, sw, ref, tot) VALUES('" & Me.LblEmpleado.Text & "', 'DEL_LINEA', '" & Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.name).Text & " - " & Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.total).Text & "','" & Format(Now, "dd/MM/yyyy HH:mm:ss") & "', " & IIf(Me.LstLines.SelectedItems(0).Text > 0, True, False) & ", " & Me.idRef & "," & Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.total).Text.Replace(",", ".") & ")")
                My.m_db.Execute("INSERT INTO logs_lines(usuario, tipo, str, fh, sw, ref, tot) VALUES('" & Me.LblEmpleado.Text & "', 'DEL', '" & Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.name).Text & " - " & Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.total).Text & "', '" & Format(Now, "dd/MM/yyyy HH:mm:ss") & "', " & IIf(Me.LstLines.SelectedItems(0).Text > 0, True, False) & ", " & Me.idRef & ", " & Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.total).Text.Replace(",", ".") & ")")
            ...
            Case "N_DOWN"
                ...
                My.m_db.Execute("INSERT INTO logs_lines(usuario, tipo, str, fh, sw, ref, tot) VALUES('" & Me.LblEmpleado.Text & "', 'DOWN', '" & Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.name).Text & " - " & Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.pvp_ud).Text & "', '" & Format(Now, "dd/MM/yyyy HH:mm:ss") & "', " & IIf(Me.LstLines.SelectedItems(0).Text > 0, True, False) & ", " & Me.idRef & ", " & Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.pvp_ud).Text.Replace(",", ".") & ")")
    ...
    Private Sub CheckStateButtons()
        ...
        If sw Then
            ...
            If My.m_opt.swNotDelete Then Me.BtN_Down.Visible = False
        End If
...
#Region "Eventos Principales"
    ...
    Private Sub m_Shell_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'Me.BtCobrar.Visible = Not My.MyHardware.cashlogy_sw
        Me.BtEditTicket.Enabled = Not My.MyHardware.cashlogy_sw
        If Me.swFinalizado Then
            Me.BtCobroDirecto.Visible = Not My.MyHardware.cashlogy_sw
            Me.BtN_Down.Visible = Not My.MyHardware.cashlogy_sw
            Me.BtN_Up.Visible = Not My.MyHardware.cashlogy_sw

            Me.BtFreeArt.Visible = Not My.MyHardware.cashlogy_sw
            Me.PnlArticulos.Visible = Not My.MyHardware.cashlogy_sw

            Me.btProductos.Visible = Not My.MyHardware.cashlogy_sw
            Me.BtDel_Line.Visible = Not My.MyHardware.cashlogy_sw

            Me.BtLastArticulo.Visible = Not My.MyHardware.cashlogy_sw
            Me.BtNewTicket.Visible = Not My.MyHardware.cashlogy_sw

            If My.MyHardware.cashlogy_sw Then Me.Tab.TabPages.Remove(Me.TabPage_Option)
        End If
        ...
        Me.pnlEstanco.Height = Me.PnlArticulos.Height

        Me.pnlEstanco.Width = Me.PnlArticulos.Width - 10
        ...
        Me.pnlLstArticulos.Height = Me.PnlArticulos.Height

        Me.pnlLstArticulos.Width = Me.PnlArticulos.Width - 10
        ...
        Me.ActiveControl = Me.LstLines

        Me.lstHistory.Visible = (Me.idMesa <> -1)
        Me.btFacturarTickets.Visible = (Me.idMesa <> -1)
        Me.lblHistoryTotal.Visible = (Me.idMesa <> -1)


        ' BLoque de pantalla
        If Not My.m_opt.swBlockAut Then Me.tmrBlock.Enabled = False


        Me.BtN_Down.Visible = Not My.m_opt.swNotDelete
        Me.BtDel_Line.Visible = Not My.m_opt.swNotDelete
        Me.BtFreeArt.Visible = Not My.m_opt.swNotDelete

        If Me.swFinalizado AndAlso My.MyHardware.cashlogy_sw Then
            Me.BtN_Down.Visible = False
            Me.BtDel_Line.Visible = False
            Me.BtFreeArt.Visible = False
        End If
...
#Region "Eventos Auxiliares"
    ...
    Private Sub BtLastArticulo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtLastArticulo.Click
        ...
        If Me.LstLines.SelectedItems.Count <= 0 OrElse My.m_opt.swNotDelete Then Exit Sub
    ...
    Private Sub m_Shell_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        ...
        Me.btHack_Click(Me.btHack, New System.EventArgs)
    End Sub
...
#Region "Categorias de Productos"
    ...
    Private Sub Load_CatProductos()
        ...
        'Dim _x As Integer = 4
        Dim _x As Integer = ((Me.PnlCategorias.Width - 150) / 120)
        ...
        sql = "SELECT * FROM db_categorias WHERE swDescatalogada=FALSE AND swExtras=FALSE "
        'If Not swOrderBYFijo Then
        sql &= " ORDER BY n_veces DESC,name ASC"
        'Else
        'sql &= " ORDER BY n_orden DESC,name ASC"
        'End If
...
#Region "Articulos de Categoria"
    ...
    Private Sub Load_ArticulosFromCat(ByVal id_cat As Integer)
        ...
        'Ajusto los productos que entran en resposive
        _x = ((Me.PnlArticulos.Width) / 120)
        _y = ((Me.PnlArticulos.Height) / 115)
        ...
        '### Limpio anteriores controltes
        For i = Me.PnlArticulos.Controls.Count - 1 To 0 Step -1
            Me.PnlArticulos.Controls(i).Dispose()
        Next

        '### Obtengo los articulos de la categoria seleccionada
        ...
        'sql &= " ORDER BY " & IIf(My.m_opt.swNotOrder, "", " db_articulos.n_veces DESC,") & "db_articulos.name ASC"
        If Not swOrderBYFijo Then
            sql &= " ORDER BY " & IIf(My.m_opt.swNotOrder, "", " db_articulos.n_veces DESC,") & "db_articulos.name ASC"
        Else
        ...
        While Not rst.EOF
            If (n >= (Me._articulos_pag * (_x * _y))) Then
                ...
                If CBool(rst("swComposicion").Value) Then
                    bt.Font = New Font("Tahoma", 12, FontStyle.Bold)
                End If
        ...
        If Me.swFinalizado Then Me.PnlArticulos.Visible = Not My.MyHardware.cashlogy_sw
        ...
        While Not rst.EOF
            If (n >= (Me._artTOP_pag * (_x * _y))) Then
                ...
                If CBool(rst("swComposicion").Value) Then
                    ...
                    '    .Location = New Point(_left + (bt.Width * i) + 5, _top + (bt.Height * j) + 5)
                    '    .FlatStyle = FlatStyle.Popup
                    '    .Tag = rst("id").Value
                    '    .Visible = True

                    '    ' Asigno Evento
                    '    AddHandler .Click, AddressOf ClickComposicion
                    'End With
...
#Region "Ticket"
    ...
    Private Sub ConfiguraTicket()
        With Me.LstLines.Columns
            ...
            .Add("compose", 0, HorizontalAlignment.Left)
            .Add("mixto", 0, HorizontalAlignment.Left)
        End With

        'If My.m_opt.ticketAvanzado Then
        Me.LstLines.Columns(lst_Columns_Tickets.ud).Width = 40
        Me.LstLines.Columns(lst_Columns_Tickets.name).Width = 170
        Me.LstLines.Columns(lst_Columns_Tickets.total).Width = 70

        Me.LstLines.Font = New Font("Tahoma", 12.0!, FontStyle.Regular)
    ...
    Private Sub ProcesaClick(ByVal id As Integer, Optional ByVal idCombinaCon As Integer = 0, Optional ByVal tipo As Integer = 0)
        ...
        Dim sCompose As String = "", sMixto As String = ""
        ...
        If Me.swFinalizado AndAlso My.MyHardware.cashlogy_sw Then Exit Sub
        ...
        Else
            ...
            If CBool(rst("swMixto").Value) Then
                With frmPaneldeVentas_Compone
                    ...
                    .lblArticulo.Text = rst("name").Value
                    .ShowDialog(Me)
                    ...
                    'If CDbl(.lblPrecio.Text) > 0 Then
                    'pvp_inc = CDbl(rst("pvp_iva").Value)
                    'pvp_inc_Base = Format(pvp_inc / CDbl("1," & Format(rst("iva").Value, "00")), "0.00#")
                    'End If

                    .Dispose()
                End With

            ElseIf rst("combinable").Value AndAlso (idCombinaCon <= 0 AndAlso tipo = 0) AndAlso Not CBool(rst("swComposicion").Value) Then
                With frmPaneldeVentas_Combina
                    ...
                    .Dispose()
                End With
            ElseIf CBool(rst("swComposicion").Value) Then
                With frmPaneldeVentas_Compone
                    .Id_CatRef = rst("id_categoria_combina").Value
                    .sCombina = rst("composicion").Value
                    .lblArticulo.Text = rst("name").Value
                    .ShowDialog(Me)
                    If .DialogResult <> Windows.Forms.DialogResult.OK Then
                        .Dispose()
                    ...
                    'For j As Integer = 0 To .lst.Items.Count

                    'Next


                    If CDbl(.lblPrecio.Text) > 0 Then
                        pvp_inc = CDbl(.lblPrecio.Text)
                        pvp_inc_Base = Format(pvp_inc / CDbl("1," & Format(rst("iva").Value, "00")), "0.00#")
                    End If
            ...
            For i = 0 To Me.LstLines.Items.Count - 1
                ...
                Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.compose).Text & "" = sCompose AndAlso _
                Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.mixto).Text & "" = sMixto AndAlso _
        ...
        If Not sw Then           'No esta agregado asi que lo agrego
            With Me.LstLines
                ...
                .Items(i).SubItems.Add(sCompose)
                .Items(i).SubItems.Add(sMixto)
    ...
    Private Sub SaveTicket(Optional ByVal kill As Boolean = False, Optional ByVal swPagoEfectivo As Boolean = True, Optional ByVal swNota As Boolean = False)
        ...
        If kill Then
            'If My.MyHardware.cashlogy_sw Then
            '    'MANDO EL COBRO
            '    frmPaneldeVentas_CobraCashlogy.DblTotal = CDbl(Me._DblTotal)
            '    frmPaneldeVentas_CobraCashlogy.ShowDialog(Me)
        ...
        For i = 0 To Me.LstLines.Items.Count - 1
            If Me.LstLines.Items(i).Text = 0 Then               'Si es NUEVO
                sql = "INSERT INTO db_tickets_lines (id_ticket,id_articulo,id_articulo_combina,id_marca,name,ud,pvp_base,iva,pvp_ud,total,swcomanda,tipo,entrada,tipo_invitacion,composicion,mixto) "
                sql &= "VALUES (" & Me.idRef & ","
                ...
                sql &= "'" & Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.compose).Text & "" & "',"
                sql &= "'" & Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.mixto).Text & "" & "'"
                sql &= ")"
                My.m_db.Execute(sql)
        ...
        If My.MyComanda.IdPortComandas > 0 AndAlso Not kill Then
            Dim strComanda As String = "", strAux As String = "", strCod As String = ""
            Dim strPrinter As String = "comandas"
            Dim strPrinter_cocina As String = "cocina", strComanda_cocina As String = ""
            Dim strPrinter_barra As String = "barra", strComanda_barra As String = ""
            Dim strPrinter_cocinaFria As String = "fria", strComanda_fria As String = ""
            Dim str As String = ""

            Dim rstAux As ADODB.Recordset = Nothing

            Dim rstCompose As ADODB.Recordset = Nothing
            Dim rstArt As ADODB.Recordset = Nothing

            ' COMPRUEBO SI TIENE CATEGORIAS DE IMPRESION
            For i = 0 To Me.LstLines.Items.Count - 1
                rstAux = My.m_db.GetRst("select printercomandas FROM db_categorias WHERE id=" & Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.id_categoria).Text)
                If rstAux.RecordCount > 0 Then
                    If (Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud).Text - Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud_original).Text) > 0 Then
                        Select Case rstAux("printercomandas").Value & ""
                            Case "COCINA"
                                If My.MyComanda.comandasRPT Then
                                    If strComanda_cocina.Length > 0 Then strComanda_cocina &= " & "
                                    strComanda_cocina &= "'  " & Format((Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud).Text - Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud_original).Text)) & " " & Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.name).Text & "' & Chr(10) & Chr(13) "
                                Else
                                    strComanda_cocina &= Format((Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud).Text - Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud_original).Text)) & " " & Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.name).Text & My.MyHardware.CodEsc_Cr
                                End If
                                '"'  1 - PIZZA' & Chr(10) & Chr(13) "
                            Case "FRIA"
                                If My.MyComanda.comandasRPT Then
                                    If strComanda_fria.Length > 0 Then strComanda_fria &= " & "
                                    strComanda_fria &= Format((Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud).Text - Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud_original).Text)) & " " & Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.name).Text & My.MyHardware.CodEsc_Cr
                                Else
                                    strComanda_fria &= "'  " & Format((Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud).Text - Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud_original).Text)) & " " & Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.name).Text & "' & Chr(10) & Chr(13) "
                                End If

                            Case "BEBIDAS"
                                If My.MyComanda.comandasRPT Then
                                    If strComanda_barra.Length > 0 Then strComanda_barra &= " & "
                                    strComanda_barra &= "'  " & Format((Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud).Text - Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud_original).Text)) & " " & Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.name).Text & "' & Chr(10) & Chr(13) "
                                Else
                                    strComanda_barra &= Format((Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud).Text - Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.ud_original).Text)) & " " & Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.name).Text & My.MyHardware.CodEsc_Cr
                                End If
                        ...
                        'Dim arr() As String = gDevelop.Lite.m_Functions.SplitString(Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.compose).Text)

                        Dim arr() As String = Nothing

                        If Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.mixto).Text.Length > 0 Then
                            arr = gDevelop.Lite.m_Functions.SplitString(Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.mixto).Text)
                            'sCompleta = ""
                            ...
                            If Not IsNothing(arr) Then
                                For k As Integer = 0 To arr.Length - 1
                                    ...
                                    If Not rstCompose.EOF Then
                                        If sCompleta.Length > 0 Then sCompleta &= " & "
                                        sCompleta &= "'   " & "- " & rstCompose("name").Value & "' & Chr(10) & Chr(13) "
                                    End If
                                Next
                        ...
                        Else
                            ...
                            arr = gDevelop.Lite.m_Functions.SplitString(Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.compose).Text)

                            If Not IsNothing(arr) Then
                                rstArt = My.m_db.GetRst("SELECT composicion FROM db_articulos WHERE id=" & Me.LstLines.Items(i).SubItems(lst_Columns_Tickets.id_art).Text)
                                Dim arrClone() As String = gDevelop.Lite.m_Functions.SplitString(rstArt("composicion").Value & "")
                            ...
                            End If
                        End If
                        If sCompleta.Length > 0 Then
                            Select Case rstAux("printercomandas").Value
                                Case "COCINA"
                                    'If strComanda_cocina.Length > 0 Then strComanda_cocina &= " & "
                                    strComanda_cocina &= sCompleta

                                Case "FRIA"
                                    'If strComanda_fria.Length > 0 Then strComanda_fria &= " & "
                                    strComanda_fria &= sCompleta

                                Case "BEBIDAS"
                                    'If strComanda_barra.Length > 0 Then strComanda_barra &= " & "
                                    strComanda_barra &= sCompleta
            ...
            ' Impresora de COCINA
            If strComanda_cocina.Length > 0 Then
                If Not My.MyComanda.comandasRPT Then
                    If My.MyHardware.CodEsc_Init.Length > 0 Then My.RawPrinterHelper.SendStringToPrinter(strPrinter_cocina, My.MyHardware.CodEsc_Init & My.MyHardware.CodEsc_Cr)
                    strCod = ""
                    str = ""
                    ...
                    str &= My.MyHardware.CodEsc_TextGrande & strComanda_cocina & My.MyHardware.CodEsc_TextNormal & My.MyHardware.CodEsc_Cr
                    ...
                    My.RawPrinterHelper.SendStringToPrinter(strPrinter_cocina, str & My.MyHardware.CodEsc_Cr)
                Else
                    Dim RptPrint As CrystalDecisions.CrystalReports.Engine.ReportDocument
                    RptPrint = New crystal
                    CType(RptPrint, crystal).DataDefinition.FormulaFields("titleName").Text = "'" & My.m_opt.company_name & "'"
                    CType(RptPrint, crystal).DataDefinition.FormulaFields("camarero").Text = "''"
                    CType(RptPrint, crystal).DataDefinition.FormulaFields("mesa").Text = "'" & Me.LblMesa.Text & "'"
                    CType(RptPrint, crystal).DataDefinition.FormulaFields("text").Text = "" & strComanda_cocina & ""

                    RptPrint.PrintOptions.PrinterName = strPrinter_cocina
                    'RptPrint.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, "c:\temp\1111111\doc.pdf")
            ...
            End If
                ...
                ' Impresora de FRIA
                If strComanda_fria.Length > 0 Then

                    'If My.MyHardware.CodEsc_Init.Length > 0 Then My.RawPrinterHelper.SendStringToPrinter(strPrinter_cocinaFria, My.MyHardware.CodEsc_Init & My.MyHardware.CodEsc_Cr)
                    'strCod = ""
                    'str = ""
                    ...
                    'str &= strComanda_fria & My.MyHardware.CodEsc_Cr
                    ...
                    'My.RawPrinterHelper.SendStringToPrinter(strPrinter_cocinaFria, str & My.MyHardware.CodEsc_Cr)
                    ...
                    CType(RptPrint, crystal).DataDefinition.FormulaFields("camarero").Text = "''"
                    CType(RptPrint, crystal).DataDefinition.FormulaFields("mesa").Text = "'" & Me.LblMesa.Text & "'"
                    CType(RptPrint, crystal).DataDefinition.FormulaFields("text").Text = "" & strComanda_fria & ""

                    RptPrint.PrintOptions.PrinterName = strPrinter_cocinaFria
                    RptPrint.PrintToPrinter(1, False, 0, 0)
                    RptPrint.Dispose()
                End If

                ' Impresora de BEBIDAS
                If strComanda_barra.Length > 0 Then
                If Not My.MyComanda.comandasRPT Then
                    If My.MyHardware.CodEsc_Init.Length > 0 Then My.RawPrinterHelper.SendStringToPrinter(strPrinter_barra, My.MyHardware.CodEsc_Init & My.MyHardware.CodEsc_Cr)
                    strCod = ""
                    str = ""
                    ...
                    str &= My.MyHardware.CodEsc_TextGrande & strComanda_barra & My.MyHardware.CodEsc_TextNormal & My.MyHardware.CodEsc_Cr
                    ...
                    My.RawPrinterHelper.SendStringToPrinter(strPrinter_barra, str & My.MyHardware.CodEsc_Cr)
                Else
                    Try
                        Dim RptPrint As CrystalDecisions.CrystalReports.Engine.ReportDocument
                        RptPrint = New crystal
                        ...
                        CType(RptPrint, crystal).DataDefinition.FormulaFields("camarero").Text = "''"
                        CType(RptPrint, crystal).DataDefinition.FormulaFields("mesa").Text = "'" & Me.LblMesa.Text & "'"
                        CType(RptPrint, crystal).DataDefinition.FormulaFields("text").Text = "" & strComanda_barra & ""

                        RptPrint.PrintOptions.PrinterName = strPrinter_barra
                        RptPrint.PrintToPrinter(1, False, 0, 0)
                        RptPrint.Dispose()
    ...
    Private Sub LoadInfo_Ticket()
        ...
        While Not rst.EOF
            With Me.LstLines
                ...
                .Items(i).SubItems.Add(rst("composicion").Value & "")
                .Items(i).SubItems.Add(rst("mixto").Value & "")
...
#Region "Impresión del Ticket"
    ...
    Public Sub PrintTicket(ByVal DblEntrega As Double, Optional ByVal OpenCajon As Boolean = True)
        ...
        If My.MyHardware.PortName = "REPORT" Then
            ...
            RptPrint.RecordSelectionFormula = "{db_tickets.id}=" & Me.idRef
            RptPrint.PrintToPrinter(1, False, 0, 0)
            RptPrint.Dispose()
        ...
        Else
            ...
            If My.MyHardware.Ancho - StrAux.Length - Me.LblFh_Alta.Text.Length < 0 Then
                StrAux &= Space(2) & Me.LblFh_Alta.Text
            Else
                StrAux &= Space(My.MyHardware.Ancho - StrAux.Length - Me.LblFh_Alta.Text.Length) & Me.LblFh_Alta.Text
            End If
            ...
            If Me.lblDto.Visible Then          'Tengo descuento
                StrAux = "Dto: " & My.MyHardware.CodEsc_TextGrande & Format(Me.lblDto.Tag, "###0.00") & My.MyHardware.CodEsc_TextNormal
                StrAux = Space(My.MyHardware.Ancho - StrAux.Length - 1) & StrAux
            ...
            If My.MyHardware.Ancho - 20 - 17 < 0 Then
                StrAux &= Space(2) & My.MyHardware.CodEsc_Subrayado_True & "% IVA" & My.MyHardware.CodEsc_Subrayado_False
            Else
            ...
            ' Resumen Final
            If My.MyHardware.Ancho - 31 < 0 Then
                StrAux = Space(1) & My.MyHardware.CodEsc_Subrayado_True & Space(27) & My.MyHardware.CodEsc_Subrayado_False
            Else
...
#Region "Estancos"
    Private Sub Load_Marcas()
        ...
        If Not Me.pnlEstanco.Visible Then Me.LstLines.Focus()
    End Sub
...
#Region "Productos"
    ...
    Private Sub Load_Productos()
        ...
        If Not Me.pnlLstArticulos.Visible Then Me.LstLines.Focus()
    End Sub
...
#End Region
    ...
    Private Sub frmPaneldeVentas_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        If e.KeyValue = Keys.Escape Then
            ...
            If Me.pnlLstArticulos.Visible Then
                Me.btEstancos_Click(Me.btProductos, New System.EventArgs)
                Exit Sub
            End If
    ...
    Private Sub picLogo_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles picLogo.DoubleClick
        My.ShellExecute(0, Nothing, "http://gdevelop.es", Nothing, Nothing, 0)
    End Sub

    Private Sub BtCobroDirecto_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles BtCobroDirecto.MouseClick
        If e.Clicks > 1 Then Me.Buttons_Click(Me.BtCobrar, New System.EventArgs)
    End Sub

    Private Sub btHack_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btHack.Click
        Me.LstLines.Select()

    End Sub

    Private Sub tmrBlock_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tmrBlock.Tick
        If Not My.m_opt.swBlockAut Then
            Me.tmrBlock.Enabled = False
            Exit Sub
        End If

        If DateDiff(DateInterval.Second, hourNow, Now) > 15 Then
            Me.tmrBlock.Enabled = False
            Dim frm As frmScreenSave = New frmScreenSave
            frm.LblTotal.Text = Me.LblTotal.Text
            frm.ShowDialog()
            Me.hourNow = Now
            frm.Dispose()
            frm = Nothing
    ...
    Private Sub frmPaneldeVentasResponsive_MouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseClick
        Me.hourNow = Now
    End Sub

    Private Sub frmPaneldeVentasResponsive_MouseMove(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseMove
        Me.hourNow = Now
    End Sub

    Private Sub LstLines_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LstLines.DoubleClick
        If Me.LstLines.SelectedItems.Count <= 0 Then Exit Sub
        If CStr(Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.compose).Text & "").Length <= 0 Then Exit Sub

        Dim rst As ADODB.Recordset = Nothing
        Dim str As String = ""
        Dim arr() As String = gDevelop.Lite.m_Functions.SplitString(Me.LstLines.SelectedItems(0).SubItems(lst_Columns_Tickets.compose).Text)
        For i As Integer = 0 To arr.Length - 1
            rst = My.m_db.GetRst("SELECT name,pvp_iva FROM db_articulos WHERE id=" & arr(i))
            If Not rst.EOF Then str &= "    - " & rst("name").Value & IIf(rst("pvp_iva").Value > 0, "(+" & rst("pvp_iva").Value & ")", "") & vbCrLf
        Next

        MsgBox(str, MsgBoxStyle.OkOnly, "Composición")
